...
const handleReaction = (event) => {
  if (event.target.id === 'like-button') {
    ...
    likeButtonElement.removeEventListener('click', handleReaction); // remove event listener from our like button
  } else {
    dislikesCount = dislikesCount + 1;
    dislikeButtonElement.textContent = 
      `${dislikesCount} dislike(s). Dislike this post!`;
    dislikeButtonElement.removeEventListener('click', handleReaction); // remove event listener from our dislike button
  }
};
```
